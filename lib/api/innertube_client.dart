...
class InnerTubeClient {
  ...
  static const List<Map<String, dynamic>> _playerClients = [
    ...
    },
    {
      ...
      'clientVersion': '6.20250312',
      'hl': 'en',
      'gl': 'US',
      '_ua':
          'com.google.android.apps.youtube.mediashell/6.20250312 (Linux; U; Android 14)',
      '_clientNameId': '95',
    },
  ];
  ...
  Future<Map<String, dynamic>> _post(
    ...
    final headers = {
      ...
      'X-YouTube-Client-Version': clientVersion ??
          (_webClient['clientVersion'] as String?) ??
          '2.20250713.00.00',
    };
  ...
  Future<List<Video>> getCategoryFeed(
    String category, {
    String region = 'IN',
  }) async =>
  ...
  Future<SearchResult> getCategoryPage(
    String category, {
    String region = 'IN',
    String? continuation,
  }) async {
  ...
  String _categoryQuery(String category, String region) {
    ...
    switch (c) {
      ...
      case 'youtube music':
        return isIn ? 'latest music india 2025' : 'youtube music hits 2025';
      case 'gaming':
  ...
  Future<List<Video>> _buildDiscoverFeed({String region = 'IN'}) async {
    ...
    for (var i = 0; i < queries.length; i += 4) {
      ...
      final results = await Future.wait(
        batch.map((q) async {
          try {
            return (await search(q, region: region)).videos;
          } catch (_) {
            return <Video>[];
          }
        }),
  ...
  Future<VideoDetails> getVideoDetails(String videoId, {String? region}) async {
    ...
    for (final raw in _playerClients) {
      ...
      futures.add(() async {
        try {
          ...
          } else if (name == 'MEDIACONNECT') {
            mediaConnectDetails = details;
          }
        } catch (e) {
          lastError = e;
          debugPrint('Client $name failed: $e');
        }
      }());
    }
    ...
    try {
      webDetails = await _fetchWebPlayer(videoId);
    } catch (e) {
      debugPrint('WEB client failed: $e');
    }

    final base =
        iosDetails ?? androidDetails ?? mediaConnectDetails ?? webDetails;
    ...
    final liveGuess = (iosDetails?.isLive ?? false) ||
        ...
        (mediaConnectDetails?.isLive ?? false) ||
        base.isLive;
    String? hls;
    if (liveGuess) {
      hls = androidDetails?.hlsUrl ??
          iosDetails?.hlsUrl ??
          mediaConnectDetails?.hlsUrl ??
          base.hlsUrl;
    } else {
      hls = iosDetails?.hlsUrl ??
          androidDetails?.hlsUrl ??
          mediaConnectDetails?.hlsUrl ??
          base.hlsUrl;
    }
    ...
    for (final d in [
      ...
      if (d != null && d.channelName.isNotEmpty) {
        bestChannel = d.channelName;
        break;
      }
    }
    ...
    for (final d in [
      ...
      if (d != null && d.captionTracks.isNotEmpty) {
        bestCaptions = d.captionTracks;
        break;
      }
    }
    String bestThumb = '';
    for (final d in [
      ...
      if (d != null && d.thumbnailUrl.isNotEmpty) {
        bestThumb = d.thumbnailUrl;
        break;
      }
    }
  ...
  Future<VideoDetails> _fetchPlayer(
    ...
    if (status != null && status != 'OK') {
      throw Exception(
        'Unplayable ($status): ${response['playabilityStatus']?['reason']}',
      );
    }
  ...
  Future<VideoDetails> _fetchWebPlayer(String videoId) async {
    ...
    final response = await _post(
      ...
      userAgent: _webClient['userAgent'] as String,
      clientNameId: '1',
      clientVersion: _webClient['clientVersion'] as String,
    );
    final status = response['playabilityStatus']?['status']?.toString();
    if (status != null && status != 'OK') {
      throw Exception('WEB Unplayable: $status');
    }
    return _parseVideoDetails(
      response,
      videoId,
  ...
  Future<List<Video>> getRelatedVideos(String videoId) async {
    try {
      ...
      debugPrint('getRelatedVideos error: $e');
      return [];
    }
  }
  ...
  Future<List<SponsorSegment>> getSponsorSegments(String videoId) async {
    ...
    try {
      ...
      final res = await _http.get(
        uri,
        headers: {'Accept': 'application/json', 'User-Agent': 'VibeTube'},
  ...
  List<VideoFormat> _parseFormats(
    ...
    for (final f in formats) {
      ...
      final initRange = f['initRange'] as Map?;
      final indexRange = f['indexRange'] as Map?;
      out.add(
        VideoFormat(
          url: url,
          ...
          width: asInt(f['width']) ?? 0,
          height: asInt(f['height']) ?? 0,
          bitrate: asInt(f['bitrate']) ?? 0,
          itag: asInt(f['itag']) ?? 0,
          ...
          hasVideo: hasVideo,
          initRangeStart: asInt(initRange?['start']),
          initRangeEnd: asInt(initRange?['end']),
          indexRangeStart: asInt(indexRange?['start']),
          indexRangeEnd: asInt(indexRange?['end']),
          contentLength: asInt(f['contentLength']) ?? 0,
          approxDurationMs: asInt(f['approxDurationMs']) ?? 0,
          fps: asInt(f['fps']) ?? 0,
          audioSampleRate: asInt(f['audioSampleRate']) ?? 0,
          audioChannels: asInt(f['audioChannels']) ?? 0,
        ),
      );
    }
    return out;
    ...
    void walk(dynamic node) {
      if (node is Map) {
        ...
        if (node['lockupViewModel'] is Map) {
          final v = _extractLockup(
            Map<String, dynamic>.from(node['lockupViewModel'] as Map),
          );
          if (v != null) videos.add(v);
        }
    ...
    try {
      ...
      if (kind == 'reelItemRenderer') {
        id = r['videoId']?.toString() ??
            r['navigationEndpoint']?['reelWatchEndpoint']?['videoId']
                ?.toString() ??
            '';
  ...
  Video? _extractVideo(Map<String, dynamic> r) {
    ...
    var isLive = false;
    for (final b in (r['badges'] as List? ?? const [])) {
      final label = b is Map
          ? '${_text(b['metadataBadgeRenderer']?['label'])} ${b['metadataBadgeRenderer']?['style']?.toString() ?? ''}'
          : '';
      if (label.toUpperCase().contains('LIVE')) isLive = true;
    }
    if (r['thumbnailOverlays'] is List) {
      for (final o in r['thumbnailOverlays']) {
        if (o.toString().toUpperCase().contains('LIVE')) isLive = true;
      }
    }
    var isShort = false;
    try {
      final nav = r['navigationEndpoint']?.toString() ?? '';
      if (nav.contains('reelWatchEndpoint') || nav.contains('/shorts/')) {
        isShort = true;
      }
    } catch (_) {}
    final dur = _parseDurationText(lengthText);
    if (lengthText.toLowerCase().contains('short') && dur.inSeconds <= 60) {
      isShort = true;
    }
    return Video(
      id: id,
